...
type HighLatitudeAdapter func(cfg Config, year int, month int, day int, currentSchedules []Schedule) []Schedule
...
func Calculate(cfg Config, year int, month int, day int) ([]Schedule, error) {
	...
	schedules, nAbnormal := calcNormal(cfg, year, month, day)

	// Apply high latitude adapter
	if nAbnormal > 0 && cfg.HighLatitudeAdapter != nil {
		schedules = cfg.HighLatitudeAdapter(cfg, year, month, day, schedules)
	}
